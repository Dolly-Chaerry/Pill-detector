...
from config import DATASET_PATH, CROPPED_PATH
from pathlib import Path
import cv2
import pandas as pd

#FOR TRAINING : segment training images
segmenter = ImageSegmenter()

print("Starting segmentation of training images...")
for file in Path(DATASET_PATH).iterdir():
    if file.is_file() and file.suffix in [".jpg", ".png", ".jpeg"]:
        print(f"Segmenting file: {DATASET_PATH}/{file.name}")
